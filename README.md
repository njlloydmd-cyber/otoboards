...
- **Cross-device sync (optional):** Sign in with Google to keep your sessions in sync across devices. This is entirely optional — the app works fully with zero account, storing everything in the browser's local storage. Signing in adds a copy in Firestore, scoped to your account only (separate from the shared Question Bank above). The first time you sign in on a given account, any existing local sessions on that device are merged with whatever's already synced for that account, not overwritten.
